...
public class CarritoControlador {
    ...
    @RequestMapping(value = "/orden/crear", method = {RequestMethod.GET, RequestMethod.POST})
    public String crearOrden(RedirectAttributes redirectAttributes) {
        try {
            Orden orden = carritoServicio.crearOrdenDesdeCarrito();
            return "redirect:/pago/" + orden.getId();
        } catch (RuntimeException error) {
            redirectAttributes.addFlashAttribute("error", error.getMessage());
            return "redirect:/carrito";
        }
